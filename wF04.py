...
class F04(Tk):
    ...
    def __init__(self, Score, IdJoueur): # Importation du paramètre score de F02
        ...
        self.attributes("-fullscreen", self.fullScreenState)  #On spécifie si la fenêtre est en mode plaine écran ou non
    ...
    # ELEMENTS GRAPHIQUES::::::::
        ...
        self.CanvasResult = Canvas(self, width=self.Largeur, height=self.Hauteur) #Création d'une nouvelle espace d'écriture
        self.ImgFondF04 = self.CanvasResult.create_image(self.Largeur // 2, self.Hauteur // 2, image=self.FondF04) #On met les paramètres de longueur et largeur de l'image de fond
        self.CanvasResult.pack(padx=5, pady=5)  # .pack sert à placer le texte
        self.CanvasResult.tag_lower(self.ImgFondF04) #Deplace l'objet
        # ...........< L A B E L S > .........................
        # ELEMENT GRAPHIQUE : <Label> = [A definir] : Afficher Score, nom, ...
        # >>>>> ??? plusieurs elements a faire !!!
        self.CanvasResult.create_text(600, 100,
                                      text="Résultat de la partie :", font='Gabriola 32 bold', fill='white') #Création de la partie du texte qui sera toujours la
        Depasse, BestScore = self.VictoireJeu() #Attribution des 2 variables de la fonction VictoireJeu
        if Depasse: #Ouverture de la boucle pour afficher 2 textes différents dans différents cas
            self.CanvasResult.create_text(300, 300,
                                       text="Bravo, vous avez battu votre record !\nVotre score est : {}\n"
                                            "Votre ancien meilleur score était : {}".format(
                                           int(self.ScoreRec), BestScore), font='Gabriola 26', fill='cyan') #Si le nouveau score est le meilleur score on affiche
        else:
            self.CanvasResult.create_text(300, 300,
                                         text="Dommage, vous ferez mieux la prochaine fois !\n Votre score est : {}\n"
                                              "Votre meilleur score est : {}".format(
                                             int(self.ScoreRec), BestScore), font='Gabriola 26', fill='cyan')#Sinon on afficher le texte de défaite
        # Création d'un widget Label (texte 'Nom')
        Label1 = Label(self, text=" Un Heros contre Galacticov ", font=('Arial', 20), fg='blue') #On personnalise le label
        ...
        self.RetourMenu = Button(self, text="Rejouer", command=self.commandeOuvreF02) #Creation du bouton et de ce qu'il fait
        ...
        self.quitButton = Button(self, text="Retourner au Menu", command=self.commandeOuvreF01) #Création d'un bouton et de ce qu'il fait
    ...
    def VictoireJeu(self): #On crée les fonctions qu'on utilise
        ...
        if self.ScoreRec >= int(tab[self.IDJoueur+ 1][3]): #Ouverture d'une boucle
            ...
            return True, int(tab[self.IDJoueur + 1][3]) #On retourne Vrai si le score a été dépasé, et le score du joueur
        else:
            print("Meilleur score non atteint")  # Pour controle
            return False, int(tab[self.IDJoueur+ 1][3]) #On retourne faux si le score n'a pas été dépassé, et le score du joueur
    #Fonctions de l'activation du pleine écran
